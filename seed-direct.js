// Seed products directly to Supabase using Service Role Key
...
async function seedProducts() {
  console.log('🚀 Seeding 4 medicine products to Supabase...\n');
  ...
  for (const product of products) {
    try {
      ...
      if (error) {
        ...
        console.log(`✅ ${product.name} - Created (ID: ${data.id.slice(0, 8)}...)`);
      }
    } catch (error) {
      console.log(`❌ ${product.name} - ${error.message}`);
    }
