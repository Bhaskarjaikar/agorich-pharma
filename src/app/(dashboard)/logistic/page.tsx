  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h1 className="text-2xl sm:text-3xl font-bold text-foreground">{t('dashboard.logistic.title')}</h1>
        ...
        {/* Error Message */}
        {error && (
          <div className="mb-4 p-4 bg-red-500/10 border border-red-500/30 rounded-lg text-red-500 dark:text-red-400">
            {error}
          </div>
        )}
        ...
        {/* Refresh Indicator */}
        {isRefreshing && (
          <div className="mb-4 flex items-center justify-center gap-2 text-indigo-500 dark:text-indigo-400 text-sm">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-indigo-500 dark:border-indigo-400"></div>
        ...
        {!loadingMetrics && metrics && (
          <div className="grid grid-cols-2 gap-2 md:gap-3 mb-4 md:mb-8">
            {/* Current Processing */}
            <Card className="rounded-3xl shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-3 md:p-6">
                <div className="flex items-center">
                  <div className="w-8 h-8 md:w-12 md:h-12 rounded-2xl flex items-center justify-center mr-2 md:mr-4 flex-shrink-0 bg-blue-500/20">
                    <Package className="w-4 h-4 md:w-6 md:h-6 text-blue-500 dark:text-blue-400" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs md:text-sm text-muted-foreground">{t('dashboard.logistic.currentlyProcessing', 'Currently Processing')}</p>
                    <p className="text-lg md:text-2xl font-bold text-foreground truncate">{metrics.active.processing || 0}</p>
                    <p className="text-[10px] md:text-xs text-muted-foreground mt-0.5 md:mt-1">{t('dashboard.logistic.activeOrders', 'Active orders')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-3xl shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-3 md:p-6">
                <div className="flex items-center">
                  <div className="w-8 h-8 md:w-12 md:h-12 rounded-2xl flex items-center justify-center mr-2 md:mr-4 flex-shrink-0 bg-amber-500/20">
                    <Truck className="w-4 h-4 md:w-6 md:h-6 text-amber-500 dark:text-amber-400" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs md:text-sm text-muted-foreground">{t('dashboard.logistic.currentlyPacking', 'Currently Packing')}</p>
                    <p className="text-lg md:text-2xl font-bold text-foreground truncate">{metrics.active.packing || 0}</p>
                    <p className="text-[10px] md:text-xs text-muted-foreground mt-0.5 md:mt-1">{t('dashboard.logistic.readyForDelivery', 'Ready for delivery')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-3xl shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-3 md:p-6">
                <div className="flex items-center">
                  <div className="w-8 h-8 md:w-12 md:h-12 rounded-2xl flex items-center justify-center mr-2 md:mr-4 flex-shrink-0 bg-indigo-500/20">
                    <Pulse className="w-4 h-4 md:w-6 md:h-6 text-indigo-500 dark:text-indigo-400" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs md:text-sm text-muted-foreground">{t('dashboard.logistic.monthlyProcessed', 'Monthly Processed')}</p>
                    <p className="text-lg md:text-2xl font-bold text-foreground truncate">{metrics.monthly.processing || 0}</p>
                    <div className="flex items-center text-[10px] md:text-xs text-emerald-500 dark:text-emerald-400 mt-0.5 md:mt-1 font-medium">
            ...
            <Card className="rounded-3xl shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-3 md:p-6">
                <div className="flex items-center">
                  <div className="w-8 h-8 md:w-12 md:h-12 rounded-2xl flex items-center justify-center mr-2 md:mr-4 flex-shrink-0 bg-emerald-500/20">
                    <ChartBar className="w-4 h-4 md:w-6 md:h-6 text-emerald-500 dark:text-emerald-400" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs md:text-sm text-muted-foreground">{t('dashboard.logistic.monthlyPacked', 'Monthly Packed')}</p>
                    <p className="text-lg md:text-2xl font-bold text-foreground truncate">{metrics.monthly.packing || 0}</p>
                    <div className="flex items-center text-[10px] md:text-xs text-emerald-500 dark:text-emerald-400 mt-0.5 md:mt-1 font-medium">
                      <TrendUp className="h-3 w-3 mr-1" />
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-3 mb-4 md:mb-8">
            {[1, 2, 3, 4].map((i) => (
              <Card key={i} className="animate-pulse">
                <CardContent className="p-3 md:p-6">
                  <div className="flex items-center">
                    <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-muted mr-2 md:mr-4"></div>
                    <div className="flex-1">
                      <div className="h-3 w-20 bg-muted rounded mb-1 md:mb-2"></div>
                      <div className="h-6 md:h-8 w-14 bg-muted rounded"></div>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-foreground">{t('dashboard.logistic.pending')}</h2>
                    <p className="text-muted-foreground text-sm">{counts.PROCESSING || 0} {t('dashboard.admin.totalOrders')}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3 max-h-[600px] overflow-y-auto">
                {grouped.PROCESSING && grouped.PROCESSING.length > 0 ? (
                  grouped.PROCESSING.map((invoice) => (
                    <div key={invoice.id} className="bg-muted border border-border rounded-lg p-3">
                      <InvoiceCard
                        invoice={invoice}
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-foreground">{t('dashboard.logistic.packed')}</h2>
                    <p className="text-muted-foreground text-sm">{counts.PACKING || 0} {t('dashboard.admin.totalOrders')}</p>
                  </div>
                </div>
              </div>
