    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/80">Loading invoice flow dashboard...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">Invoice Flow Dashboard</h1>
              <p className="text-white/70 text-sm mt-1">Track and manage complete invoice lifecycle</p>
            </div>
          </div>
        ...
        {metrics && (
          <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div className="bg-white/10 border border-white/20 rounded-xl p-4 text-white shadow-lg">
              <p className="text-sm text-white/70">Total Invoice Value</p>
              <p className="text-2xl font-bold mt-2">{formatCurrency(metrics.totalGrandTotal)}</p>
              <p className="text-xs text-white/60 mt-2">Across {metrics.totalInvoices || 0} invoices</p>
            </div>
            <div className="bg-white/10 border border-white/20 rounded-xl p-4 text-white shadow-lg">
              <p className="text-sm text-white/70">Amount Collected</p>
              <p className="text-2xl font-bold mt-2">{formatCurrency(metrics.totalPaidAmount)}</p>
              <p className="text-xs text-white/60 mt-2">Includes delivered &amp; paid receipts</p>
            </div>
            <div className="bg-white/10 border border-white/20 rounded-xl p-4 text-white shadow-lg">
              <p className="text-sm text-white/70">Outstanding Balance</p>
              <p className="text-2xl font-bold mt-2">{formatCurrency(metrics.totalOutstanding)}</p>
              <p className="text-xs text-white/60 mt-2">{metrics.partialDueCount || 0} invoices with balance due</p>
            </div>
            <div className="bg-white/10 border border-white/20 rounded-xl p-4 text-white shadow-lg">
              <p className="text-sm text-white/70 mb-2">Payment Breakdown</p>
              <div className="space-y-1 max-h-24 overflow-y-auto pr-1">
                {(() => {
                  ...
                  return entries.map(([mode, amount]) => (
                    <div key={mode} className="flex items-center justify-between text-xs text-white/80">
