...
export interface Invoice {
  ...
  invoice_date: string
  due_date: string
  status: 'DRAFT' | 'SENT' | 'PROCESSING' | 'PACKING' | 'DELIVERED' | 'PARTIAL_PAID' | 'PAID' | 'OVERDUE' | 'WAITING_FOR_APPROVAL'
  subtotal: number
  total_gst: number
  grand_total: number
  created_at?: string
  ...
  // NEW GST Order Flow Fields
  order_id?: string | null
  draft_number?: string | null  // NEW: Sequential draft number DRAFT-XXXX
  invoice_no?: string | null  // New format AGR/YYYY-YY/XXXX
  advance_paid?: number | null  // 50% advance amount
  balance_due?: number | null   // Remaining COD amount
...
export default function InvoicesPage() {
  ...
  useEffect(() => {
    ...
    try {
      const storedProfile =
        ...
        localStorage.getItem('distributor_profile') ||
  ...
  // Function to load invoices from both Supabase and localStorage
  const loadInvoices = async () => {
    try {
      ...
      try {
        ...
        if (response.ok && contentType?.includes('application/json')) {
          ...
          if (data.invoices && data.invoices.length > 0) {
            ...
            // Save API invoices to localStorage for offline use and dashboard visibility
            localStorage.setItem('distributor_invoices', JSON.stringify(allInvoices))
            console.log('✅ Saved API invoices to localStorage')
          }
        } else {
      ...
      // If no Supabase invoices, try distributor-specific localStorage
      if (allInvoices.length === 0) {
        const savedInvoices = JSON.parse(localStorage.getItem('distributor_invoices') || '[]')
      ...
      // Also check old 'invoices' key as fallback and migrate if needed
      if (allInvoices.length === 0) {
        try {
          const oldData = localStorage.getItem('invoices')
          if (oldData) {
            const parsedOldData = JSON.parse(oldData)
            if (parsedOldData.length > 0) {
              console.log('📱 Migrating old invoices to distributor_invoices key')
              allInvoices = parsedOldData
              localStorage.setItem('distributor_invoices', JSON.stringify(allInvoices))
            }
          }
        } catch (err) {
          console.error('❌ Error migrating old invoices:', err)
        }
      }
  ...
  */
  ...
  const generateUpiLink = (invoice: Invoice) => {
    ...
    const identifier = invoice.invoice_number || invoice.order_id || invoice.id
    const transactionRef = `INV-${identifier}-${Date.now()}`
    const note = `Payment for ${invoice.invoice_number ? 'Invoice' : 'Order'} ${identifier}`
  ...
  const handleUpiPayment = (invoice: Invoice) => {
    ...
    const identifier = invoice.invoice_number || invoice.order_id || invoice.id
    const transactionId = `INV-${identifier}-${Date.now()}`
  ...
  useEffect(() => {
    const savedView = localStorage.getItem('distributor-invoices-view') as ViewType
  ...
  const handleViewChange = (view: ViewType) => {
    setCurrentView(view)
    localStorage.setItem('distributor-invoices-view', view)
  }
  ...
  const filteredInvoices = invoices.filter(invoice => {
    ...
    const matchesSearch = 
      ...
      (invoice.order_id?.toLowerCase().includes(searchLower) || false) ||
      (invoice.draft_number?.toLowerCase().includes(searchLower) || false) ||
      (invoice.invoice_no?.toLowerCase().includes(searchLower) || false) ||
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      await exportInvoicesToExcel(filteredInvoices, 'distributor-invoices')
  ...
  const handleEditInvoice = (invoice: Invoice) => {
    ...
    router.push(`/distributor/create-invoice?edit=true&invoiceId=${invoice.id}`)
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 mx-auto mb-6 border-primary"></div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {showPaymentModal && selectedInvoiceForPayment && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            {!upiPaymentInitiated ? (
              // Online Payment Options and Manual Entry Form
              <>
                {/* Online Payment Options */}
                <div className="space-y-3 mb-4">
                  {/* Razorpay Payment Button */}
                  <RazorpayPaymentButton
                    amount={selectedInvoiceForPayment.grand_total}
                    invoiceId={selectedInvoiceForPayment.id}
                    invoiceNumber={selectedInvoiceForPayment.invoice_number}
                    customerName={profile?.business_name || profile?.user_name || undefined}
                    ...
                    className={`w-full font-semibold py-6 text-lg shadow-lg hover:shadow-xl transition-all duration-300 ${darkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        {/* Compact Header */}
        <div className="mb-3">
          <div className="flex flex-col sm:flex-row justify-end items-start sm:items-center mb-3 gap-2">
            <div className="flex flex-row gap-2 w-full sm:w-auto">
              <Button
                variant="outline"
                size="sm"
                onClick={() => setTheme(darkMode ? 'light' : 'dark')}
                className="flex-1 sm:w-auto text-xs"
              >
                {darkMode ? <Sun className="w-3 h-3" weight="fill" /> : <Moon className="w-3 h-3" weight="fill" />}
              </Button>
              <Button
                onClick={() => router.push('/distributor/create-invoice')}
                size="sm"
