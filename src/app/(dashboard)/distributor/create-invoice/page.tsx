...
export default function CreateInvoicePage() {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<'razorpay' | 'upi' | 'bank' | 'cod'>('razorpay')
  ...
  const calculateRate = (product: Product) => {
    // Distributor gets 60% of MRP!
    if (product.distributor_price) {
      return product.distributor_price
    }
    return product.mrp ? Math.round(product.mrp * 0.6) : 0
  }
  ...
  // Helper function to convert cart items to order items for GST Order Flow
  const getOrderItems = useCallback(() => {
    return cartItems.map((item) => {
      const rate = calculateRate(item.product)
      const amountBeforeTax = rate * item.quantity
      ...
      return {
        ...
        hsn_code: '3004', // Default HSN code for medicines
        quantity: item.quantity,
        unit: item.product.pack_size?.toLowerCase().includes('strip') ? 'strips' : 'units',
        rate_per_unit: rate,
    ...
    try {
      ...
      if (isEditMode && editingInvoice?.id) {
        ...
        const payload = {
          ...
          items: itemsForApi,
          notes: 'Invoice created via distributor dashboard',
        }
  ...
  }, [cartItems, user, getInvoiceNumber])
        ...
        try {
          ...
          if (data && data.verified && data.status === 'SUCCESS') {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                router.push('/distributor/invoices?payment=success')
              }, 2000)
            }, 1000)
      ...
      <div ref={containerRef} className="hidden md:flex h-[calc(100vh-200px)]">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1.5 mt-1.5">
                              ...
                              {product.isPrescriptionRequired && (
                                <Badge className="text-[10px] px-1.5 py-0 bg-rose-500/20 text-rose-300 border-rose-500/30">
                                  Rx
                                </Badge>
                              )}
                            </div>
                          ...
                          {cartItems.find(item => item.product.id === product.id) ? (
                            <div className="flex items-center gap-1 ml-2">
                              <button
                                ...
                                className="w-6 h-6 rounded bg-secondary hover:bg-secondary/80 flex items-center justify-center text-foreground"
                              >
                                <Minus className="w-3 h-3" />
                              </button>
                              <span className="w-6 text-center text-xs text-foreground font-medium">
                      ...
                      cartItems.map((item, index) => {
                        ...
                        return (
                          <tr key={item.product.id} className="hover:bg-gray-50">
                            ...
                            <td className="border border-border p-2 text-right text-foreground">{formatCurrency(item.product.mrp || 0)}</td>
                            <td className="border border-border p-2 text-right text-foreground">{formatCurrency(rate)}</td>
                            <td className="border border-border p-2 text-center text-foreground">5%</td>
                            <td className="border border-border p-2 text-right text-foreground">{formatCurrency(totalWithGST)}</td>
                          </tr>
                        )
                      })
                    ) : (
                      <tr>
                        <td colSpan={10} className="border border-border p-4 text-center text-muted-foreground">
                          No items added yet
                        </td>
                      </tr>
                    )}
        ...
        <div className="block md:hidden h-[calc(100vh-140px)] overflow-auto">
          ...
          {activeTab === 'products' && (
                      ...
                      <CardContent className="p-2">
                        <div className="space-y-1">
                          ...
                          <div className="flex items-start space-x-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className={`font-semibold text-sm leading-tight ${darkMode ? 'text-white' : 'text-slate-900'}`}>{product.name}</h3>
                              <p className={`text-[11px] ${darkMode ? 'text-white/60' : 'text-slate-500'}`}>{product.manufacturer}</p>
                            ...
                            {cartItems.find(item => item.product.id === product.id) ? (
                              <div className="flex items-center gap-1">
                                <Button
                                  ...
                                  className="h-6 w-6 p-0 border-orange-500/50 bg-orange-500/10 text-orange-300 hover:bg-orange-500/20"
                                >
                                  <Minus className="w-3 h-3" />
                                </Button>
                                <input
            ...
            <div className="shrink-0 bg-slate-900/95 border-t border-white/10 p-2">
              <Pagination />
            </div>
          </div>
        )}
