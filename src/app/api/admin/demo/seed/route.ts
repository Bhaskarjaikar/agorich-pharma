  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      return NextResponse.json(
        { error: 'Demo seed endpoint is disabled in production. Use this only in development/testing environments.' },
        { status: 403 }
      )
    }
    ...
    for (const rid of createdRetailerIds) {
      ...
      for (let k = 0; k < nInvoices; k++) {
        ...
        if (status === 'PAID') {
          ...
          advancePaid = Math.round(grandTotal * randomInt(20, 60) / 100)
    ...
    const { data: allProducts } = await supabase.from('products').select('id, name').limit(nProducts)
    const { data: distributors } = await supabase.from('profiles').select('id').eq('role', 'DISTRIBUTOR').limit(5)
    ...
    if (distributorIds.length === 0) {
      ...
      if (adminProfile) distributorIds.push(adminProfile.id)
    }

    let batchesCreated = 0
    for (const product of allProducts || []) {
      for (const distributorId of distributorIds) {
        const expiryMonthsAhead = randomInt(3, 24)
        const expiryDate = new Date()
        expiryDate.setMonth(expiryDate.getMonth() + expiryMonthsAhead)
        const mfgDate = new Date()
        mfgDate.setMonth(mfgDate.getMonth() - randomInt(1, 6))

        const availableQty = randomInt(10, 200)
        const reservedQty = randomInt(0, Math.floor(availableQty * 0.3))

        const { error: batchErr } = await supabase.from('inventory_batches').insert({
          product_id: product.id,
          distributor_id: distributorId,
          batch_number: `BATCH-${product.id.slice(0, 4).toUpperCase()}-${randomInt(1000, 9999)}`,
          expiry_date: expiryDate.toISOString().split('T')[0],
          available_qty: availableQty,
          reserved_qty: reservedQty,
          mfg_date: mfgDate.toISOString().split('T')[0],
        })
        if (!batchErr) batchesCreated++
      }
    }

    return NextResponse.json({ retailers: createdRetailerIds.length, invoices: invoicesCreated, batches: batchesCreated }, { status: 200 })
  } catch (error: unknown) {
