    ...
    const fetchProfile = async () => {
      ...
      const { data, error: selErr } = await supabase
        ...
        .select(`
          ...
          aadhar_number, pan_number, profile_photo, role
        `)
        .eq('id', uid)
        .single()
      if (cancelled) return
      
      // If user is DISTRIBUTOR, skip onboarding and redirect to distributor dashboard
      if (data?.role === 'DISTRIBUTOR') {
        console.log('[Onboarding] User is DISTRIBUTOR, skipping onboarding')
