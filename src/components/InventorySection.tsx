...
import { AlertTriangle, Plus, Edit, Trash2, Package, Upload, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function InventorySection({ user }: InventorySectionProps) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Only super admins can manage inventory.</CardDescription>
        </CardHeader>
      </Card>
    )
  }
  ...
  return (
    <Card className="border-0 bg-transparent">
      <CardHeader className="bg-gradient-to-r from-indigo-500/20 to-purple-500/20 px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button 
              ...
              type="button"
              className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 text-white cursor-pointer"
            >
      ...
      <CardContent className="p-6">
        ...
        {products.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-indigo-100 mb-4">Get started by adding your first product to the inventory.</p>
            <Button 
              onClick={openAddProductModal} 
            ...
            <Table className="min-w-full">
              <TableHeader>
                <TableRow className="border-white/20">
                  <TableHead className="text-white/80 w-12">
                    <input
                      type="checkbox"
                  ...
                  <TableHead className="text-white/80 min-w-[200px]">Product Name</TableHead>
                  <TableHead className="text-white/80 min-w-[80px]">Stock</TableHead>
                  <TableHead className="text-white/80 min-w-[100px]">MRP</TableHead>
                  <TableHead className="text-white/80 min-w-[120px]">Agorich Price</TableHead>
                  <TableHead className="text-white/80 min-w-[80px]">Margin</TableHead>
                  <TableHead className="text-white/80 min-w-[120px]">Pack Size</TableHead>
                  <TableHead className="text-white/80 min-w-[120px]">Batch</TableHead>
                  <TableHead className="text-white/80 min-w-[120px]">Mfg</TableHead>
                  <TableHead className="text-white/80 min-w-[100px]">Expiry</TableHead>
                  <TableHead className="text-right text-white/80 min-w-[150px]">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {paginatedProducts.map((product) => (
                  <TableRow key={product.id} className="border-white/20 hover:bg-white/5">
                    <TableCell>
                      <input
                        type="checkbox"
                    ...
                    <TableCell className="font-medium text-white whitespace-nowrap">{product.name || 'N/A'}</TableCell>
                    ...
                    <TableCell className="text-white whitespace-nowrap">₹{(product.mrp ?? 0).toFixed(2)}</TableCell>
                    <TableCell className="text-white whitespace-nowrap">₹{(product.agorich_price ?? 0).toFixed(2)}</TableCell>
                    <TableCell className="text-white/80 whitespace-nowrap">
                      {product.margin ? `${Number(product.margin).toFixed(1)}%` : '-'}
                    </TableCell>
                    <TableCell className="text-white/80 whitespace-nowrap">{product.pack_size || '-'}</TableCell>
                    <TableCell className="text-white/80 whitespace-nowrap">{product.batch_number || '-'}</TableCell>
                    <TableCell className="text-white/80 whitespace-nowrap">
            ...
            {products.length > 0 && (
              <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2 text-sm text-white/70">
                  ...
                  <span className="text-white/50">({startIndex + 1}-{Math.min(endIndex, products.length)})</span>
                </div>
