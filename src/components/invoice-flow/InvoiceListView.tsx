...
import { CaretDown, CaretRight, Phone, ChatCircle, Eye } from '@phosphor-icons/react'
          ...
          >
            ...
            >
              <div className="flex-shrink-0">
                {isExpanded ? (
                  <CaretDown className="w-4 h-4 text-white/60" />
                ) : (
                  <CaretRight className="w-4 h-4 text-white/60" />
                )}
              </div>
              
              <div className="flex-1 grid grid-cols-12 gap-4 items-center">
                <div className="col-span-3">
                  <span className={`text-white font-medium text-sm ${invoice.status === 'DRAFT' ? 'text-amber-400' : ''}`}>
