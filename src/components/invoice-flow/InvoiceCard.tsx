...
import { ChatCircle, CheckCircle, Truck, CreditCard, Clock, Phone, Spinner } from '@phosphor-icons/react'
...
function InvoiceCard({
  ...
  const getCustomerTypeBadge = () => {
    const role = invoice.customer_profile?.role
    if (role === 'DISTRIBUTOR') {
      return (
        <Badge className="bg-emerald-600/10 text-emerald-600 dark:text-emerald-300 border-emerald-400/30 text-xs px-2 py-0.5">
          Distributor
        </Badge>
      )
    }
    if (role === 'RETAILER') {
      return (
        <Badge className="bg-purple-600/10 text-purple-600 dark:text-purple-300 border-purple-400/30 text-xs px-2 py-0.5">
          Retailer
        </Badge>
      )
    }
  ...
  return (
    <motion.div
      ...
      className="bg-card border border-border rounded-lg p-4 mb-3 hover:bg-muted transition-colors"
    >
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <h4 className={`font-bold text-sm mb-1 ${invoice.status === 'DRAFT' ? 'text-amber-500 dark:text-amber-400' : 'text-foreground'}`}>
      ...
      {/* Amount */}
      <div className="mb-3">
        <p className="text-2xl font-bold text-foreground">₹{Number(invoice.grand_total).toFixed(2)}</p>
        <p className="text-xs text-muted-foreground mt-1">{t('invoice.date')}: {formatDate(invoice.invoice_date)}</p>
      </div>
      ...
      {(invoice.status === 'DELIVERED' || invoice.status === 'PAID') && invoice.payment_amount && (
        <div className="mb-3 p-2 bg-green-500/10 rounded border border-green-500/30">
          ...
          {invoice.authorized_person_name && (
            <p className="text-xs text-green-500/80 dark:text-green-300/80 mt-1">
      ...
      <div className="mb-3 space-y-1 text-xs text-muted-foreground">
        {invoice.whatsapp_sent_at && (
          <p className="flex items-center gap-1.5">
            <ChatCircle className="w-3 h-3 flex-shrink-0" />
      ...
      <div className="flex flex-col gap-2">
        ...
        {invoice.status === 'SENT' && onConfirmOrder && (
          ...
          >
            {isConfirmed ? (
              ...
              <>
                <Spinner className="w-3 h-3 mr-1 animate-spin" weight="bold" />
        ...
        {onView && (
          <Button
            ...
            className="w-full text-foreground text-xs"
          >
