...
export default function InvoiceTableView({
  ...
  const getCustomerTypeBadge = (role?: string | null) => {
    if (role === 'DISTRIBUTOR') {
      return (
        <Badge className="bg-emerald-600/20 text-emerald-300 border-emerald-400/30 text-xs px-2 py-0.5">
          Distributor
        </Badge>
      )
    }
    if (role === 'RETAILER') {
      return (
        <Badge className="bg-purple-600/20 text-purple-300 border-purple-400/30 text-xs px-2 py-0.5">
          Retailer
        </Badge>
      )
    }
      ...
      <Table className="w-full">
        <TableHeader>
          <TableRow className="border-b border-white/20 hover:bg-transparent">
            ...
            <TableHead className="text-left p-3 text-white/80">Type</TableHead>
            <TableHead className="text-left p-3 text-white/80">
              <Button
                variant="ghost"
        ...
        <TableBody>
          {sortedInvoices.map((invoice) => {
            ...
            const invoiceId = invoice.invoice_number || (invoice as { order_id?: string }).order_id || 'Draft-ID'

            return (
              <TableRow key={invoice.id} className="border-b border-white/10 hover:bg-white/5">
                <TableCell className="p-3">
                  <span className={`font-medium ${invoice.status === 'DRAFT' ? 'text-amber-400' : 'text-white'}`}>
                  {invoiceId}
                </span>
                </TableCell>
                <TableCell className="p-3 text-white/80">{customerName}</TableCell>
                <TableCell className="p-3">
                  {getCustomerTypeBadge(invoice.customer_profile?.role)}
                </TableCell>
